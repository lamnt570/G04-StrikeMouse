...
public class Controller {
	...
	private static final float sizeWidthMax = 0.25f; // width of the biggest
														// hole
	private static final float sizeHeightMax = 0.25f; // height of the biggest
														// hole
	private static final float middleX = 0f; // coordinate of middle hole in Ox
												// axis
	private static final float middleY = -5 / 16f; // coordinate of middle hole
													// in Oy axis
	private static final float deltaX = 5 / 16f; // distance between two holes
													// in Ox axis
	private static final float deltaY = 0.25f; // distance between two holes in
												// Oy axis
			...
			for (int j = 0; j < 3; j++)
				for (int k = 0; k < 6; k++) {
					holesImages[i][j][k] = new Sprite(
							Assets.instance.holes.holes[k]);
					setImageInfo(holesImages[i][j][k], sizeRatio[i]
							* sizeWidthMax, sizeRatio[i] * sizeHeightMax,
							middleX + deltaXRatio[i] * (j - 1) * deltaX,
							middleY + deltaYRatio[i] * deltaY);
				}
	}
